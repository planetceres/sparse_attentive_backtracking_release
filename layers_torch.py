...
""" 
  File "./sparse_attentive_backtracking_release/layers_torch.py", line 20, in normal
...
class Sparse_attention(nn.Module):
    ...
    def forward(self, attn_s):
        ...
        delta_fill = torch.unsqueeze(delta, 1)
        delta_fill = delta_fill.repeat(1, time_step)
        attn_w = attn_s - delta_fill
        attn_w = torch.clamp(attn_w, min=0)
        attn_w_sum = torch.sum(attn_w, dim=1)
        attn_w_sum = attn_w_sum + eps

        attn_w_sum_fill = torch.unsqueeze(attn_w_sum, 1)
        attn_w_sum_fill = attn_w_sum_fill.repeat(1, time_step)
        attn_w_normalize = attn_w / attn_w_sum_fill
...
class self_LSTM_sparse_attn_predict(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, truncate_length=100, predict_m=10,
                 block_attn_grad_past=False, attn_every_k=1, top_k=5):
        ...
        self.w_t = nn.Parameter(normal(torch.Tensor(self.hidden_size * 2, 1), mean=0.0,
