...
if d.model == "sparseattn":
    rnn = self_LSTM_sparse_attn(input_size, rnn_dim, num_layers, num_classes, truncate_length=truncate_length,
                                top_k=top_k, attn_every_k=attn_every_k)  # , block_attn_grad_past=True)
elif d.model == "trunc":
    ...
    rnn = self_LSTM_sparse_attn_predict(input_size, rnn_dim, num_layers, num_classes, truncate_length=truncate_length,
                                        top_k=top_k, attn_every_k=attn_every_k, predict_m=predict_m)
...
def evaluate_valid(valid_x, valid_y, hist_valid_loss, hist_part_loss, hist_part_acc):
    ...
    for x in valid_x:
        ...
        acc = (outputs.max(dim=2)[1][:, -10:].data == torch.squeeze(labels[:, -10:, :].data).sum())
        print(acc.shape)
        acc = acc * 1.0 / (batch_size * 10)
        valid_acc.append(acc.cpu().data.numpy())
        ...
        part_valid_loss.append(float(part_loss.data.item()))
    ...
    print("last batch of valid outputs ", (outputs.max(dim=2)[1][:, -1].data[0]))
    print("last batch of targets ", labels[:, -10:, :].data[0])
    avg_part_acc = numpy.asarray(valid_acc).mean() * 100


    log_line = model_log + 'copyiing task, T: ' + str(T) + '  rnn dim ' + str(
        rnn_dim) + ' Epoch [%d/%d] Best valid loss: %.3f, average Loss: %.3f,  part loss: %.3f, average accuracy: %.3f, validation ' % (
               epoch, num_epochs, hist_valid_loss, avg_valid_loss, avg_part_loss, avg_part_acc) + '\n'
...
for epoch in range(num_epochs):
    ...
    for x in train_x:
        ...
        if (i + 1) % 50 == 0:
            ...
            epoch, num_epochs, i + 1, loss.data, t)
            if (i + 1) % 500 == 0:
        ...
        if ((i + 1) % 200 == 0):
            hist_valid_loss, hist_part_loss, hist_part_acc = evaluate_valid(test_x, test_y, hist_valid_loss,
                                                                            hist_part_loss, hist_part_acc)
            if d.model == "sparseattn":
    ...
    hist_valid_loss, hist_part_loss, hist_part_acc = evaluate_valid(test_x, test_y, hist_valid_loss, hist_part_loss,
                                                                    hist_part_acc)
